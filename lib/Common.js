...
const CommonFncts = (() => {
	...
	CommonFncts.getBaseUrl = () => {
		return baseUrl;
	};
	
	CommonFncts.setBaseUrl = (url) => {
		baseUrl = url;
	};
	
	CommonFncts.getDisplayName = () => {
		return displayName;
	};
	
	CommonFncts.errorHandler = (err) => {
		console.log("ErrorHandler", err);
	};
	
	CommonFncts.afterEachTest = async (test) => {
		if (failureFatal) {
	...
	CommonFncts.startApp = async (overrideUrl) => {
		...
		USAGE:  CommonFncts.startApp();
		----------------------------------------------------------------------------------------------------------------- */
		if (!initialized) {
			console.log('CommonFnctsApp not initialized!: Please make a call to init prior to trying to use module.');
		...
		// 	await CommonFncts.clickElementByCSS('.message-close');
		// }*/
		
		// wait for the CommonFncts logo to be displayed
		await CommonFncts.waitForObjectLoad('.login-page-boxes', waitLong, 1000, true);
		// await CommonFncts.waitForObjectLoad('a[title*=\'Welcome\']', waitShort, 500, true);
		await CommonFncts.validateDisplayedTextEquals('.accountform.login-box>h1', 'Log in');
		
		console.log('---> run browser: application successfully started, the loaded url is "' + url + '"');
	};
	
	CommonFncts.login = async () => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		ACCEPTED PARAMETER(S) AND VALUES:   none - all of these are gathered from the *.config.json file)
		USAGE:  CommonFncts.login();
		----------------------------------------------------------------------------------------------------------------- */
		// if (harnessObj.getUserRole() === 'siteadmin') {
		
		//CraigList Login
		if (harnessObj.getUserRole() === 'craigListUser') {
			//1. find the fields & type the login name & password
			await CommonFncts.setTextFieldValueByCSS('#inputEmailHandle', username);
			await CommonFncts.setTextFieldValueByCSS('#inputPassword', password);
			
			//2. click Log In button
			await CommonFncts.clickElementByCSS('#login');
			
			//3. wait for the adds/listings grid to load --- user is logged in
			await CommonFncts.waitForObjectLoad('.account-header>a:nth-child(3)', waitLong, 500, true);
			await CommonFncts.validateDisplayedTextEquals('.account-header>a:nth-child(3)', 'home of listings@hopestreet.ca');
			await CommonFncts.waitForObjectLoad('table[class*=accthp_postings][role=grid]', waitShort, 500, true);
			
			console.log('---> login: successfully logged in as ' + harnessObj.getUserRole() + '\n');
		} else {
			//CommonFncts LOGIN
			// select the 'Local login' option
			await CommonFncts.clickElementByCSS('.user-login-form ul>li.last>a');
			await driver.sleep(500);
			
			//1. find the fields & type the login name & password
			await CommonFncts.setTextFieldValueByCSS('#edit-name', username);
			await CommonFncts.setTextFieldValueByCSS('#edit-pass', password);
			
			//2. click Log In button
			await CommonFncts.clickElementByCSS('#edit-submit');
			
			//3. wait for the admin toolbars to load --- user is logged in
			await CommonFncts.waitForObjectLoad('#toolbar-administration', waitShort, 500, true);
			await CommonFncts.waitForObjectLoad('.toolbar-bar', waitShort, 100, true);
			await CommonFncts.waitForObjectLoad('.toolbar-menu-administration', waitShort, 100, true);
	...
	CommonFncts.logout = async () => {
		/* -----------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will log the user out of the system
	
		ACCEPTED PARAMETER(S) AND VALUES:   none - all of these are gathered from the *.config.json file)
		USAGE:  CommonFncts.logout();
		----------------------------------------------------------------------------------------------------------------- */
		// await driver.findElement(By.css('a[class*=tools-help]'));
		
		await CommonFncts.waitForObjectLoad('a[class*=tools-help]', waitLong, 500, true);
		await CommonFncts.mouseHover('a[class*=tools-help]');
		driver.sleep(500);
		
		const logOutElement = await driver.findElement(By.css('a[class*=tools-help]'));
		await CommonFncts.clickElementByCSS('a[class*=logout]');
	...
	CommonFncts.navigateThroughAdminMenu = (menuName, submenuName, expectedCssLocator) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:
		//HOME BUTTON
		CommonFncts.clickTopMenuItems('Home', null, '#workspaceOverduePanelHeading1');
	
		//ASSESSMENT MENU -> Initiate
		CommonFncts.clickTopMenuItems('Assessment', 'Initiate', '#initiateInspectionPanelHeading');
		//ASSESSMENT MENU -> Tour Exports
		CommonFncts.clickTopMenuItems('Assessment', 'Tour Exports', '#TourExportPanelHeading');
		//STATEMENT OF CONCERN MENU
		CommonFncts.clickTopMenuItems('Statement of Concern', null, '??????????????????????????');
		//INITIATE MENU (no id available for this one) -> New Application
		CommonFncts.clickTopMenuItems('Initiate', 'New Application', '.pipeline-container');
		//CONSTRUCT MENU -> Water Code of Practice
		CommonFncts.clickTopMenuItems('Construct', 'Water Code of Practice', '#codeOfPracticePanelHeading');
		//CONSTRUCT MENU -> Tour Submission
		CommonFncts.clickTopMenuItems('Construct', 'Tour Submission', '#AuthorizationSearchPanelHeading');
		//CONSTRUCT MENU -> Notification -> Pipeline -> New Line
		CommonFncts.clickTopMenuItems('Construct', 'New Line', '#newNotificationPanelHeading');
		//CONSTRUCT MENU -> Notification -> Pipeline -> Liner Installation
		CommonFncts.clickTopMenuItems('Construct', 'Liner Installation', '#newNotificationPanelHeading');
		//CONSTRUCT MENU -> Notification -> Pipeline -> Liner Test
		CommonFncts.clickTopMenuItems('Construct', 'Line Test', '#newNotificationPanelHeading');
		//OPERATE MENU -> Authorization Amendment
		CommonFncts.clickTopMenuItems('Operate', 'Authorization Amendment', '#contactInformationPanelHeading');
		//OPERATE MENU -> Notification -> Flaring
		CommonFncts.clickTopMenuItems('Operate', 'Flaring', '#newSubmissionPanelHeading');
		//OPERATE MENU -> Notification -> Venting
		CommonFncts.clickTopMenuItems('Operate', 'Venting', '#newSubmissionPanelHeading');
		//CLOSE MENU -> Reclamation
		CommonFncts.clickTopMenuItems('Close', 'Reclamation', '.row.aer-workspace ol .active');
		----------------------------------------------------------------------------------------------------------------- */
		it('show site administration toolbar menu', async () => {
			//check if admin toolbar is displayed
			const adminToolbarElements = await CommonFncts.findElementsByCSS('#toolbar-item-administration-tray.is-active');
			if (adminToolbarElements.length === 0) {
				await CommonFncts.clickElementByCSS('#toolbar-item-administration');
			}
		});
		...
		it(topMenuAction + ' the ' + menuName + ' menu', async () => {
			...
			switch (menuName) {
				case 'AdminTools':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=toolbar-tools-help]');  //mouse hover top menu -> Admin Tools
					} else {
				...
				case 'Content':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=admin-content]');
				...
				case 'Structure':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=admin-structure]');
					} else {
				...
				case 'Appearance':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=themes-page]');
					} else {
				...
				case 'Extend':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=modules-list]');
					} else {
				...
				case 'Configuration':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=toolbar-icon-system-admin-config]:not([class*=config-])');
					} else {
				...
				case 'Teams':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=microsession-index]');
					} else {
				...
				case 'People':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=user-collection]');
					} else {
				...
				case 'Reports':
					if (submenuName) {
						await CommonFncts.mouseHover('a[class*=admin-reports]');
					} else {
			...
			it('click the ' + submenuName + ' submenu', async () => {
				switch (submenuName) {
					...
					case 'Index':
						...
						await CommonFncts.mouseHover('a[class*=toolbar-tools-help]');
					...
					case 'FlushCSSAndJavaScript':
						...
						await CommonFncts.mouseHover('a[class*=tools-flush]:not([class*=flush-])');
					...
					case 'DevelSettings':
						...
						await CommonFncts.mouseHover('a[class*=admin-development]');
						//click 3rd lvl sub-menu
					...
					case 'ChangeRecordsForDrupalCore':
						...
						await CommonFncts.mouseHover('a[class*=drupalorg]');
						//click 3rd lvl sub-menu
					...
					case 'Article':
						...
						await CommonFncts.mouseHover('a[class*=add-content]');
						//click 3rd lvl sub-menu
					...
					case 'Audio':
						...
						await CommonFncts.mouseHover('a[class*=add-media]');
						//click 3rd lvl sub-menu
		...
		// CommonFncts.checkErrorsExist(false);
		
		const clickTopMenuItem = async (menuCssLocator) => {
			await driver.findElement(By.css(menuCssLocator));
			await CommonFncts.clickElementByCSS(menuCssLocator);
		};
		
		const clickTopSubMenuItem = async (subMenuCssLocator) => {
			await driver.sleep(300);
			await driver.findElement(By.css(subMenuCssLocator));
			await CommonFncts.clickElementByCSS(subMenuCssLocator);
		};
		
		//wait for returned object
		it('"' + expectedCssLocator + '" object displayed', async () => {
			await CommonFncts.waitForObjectLoad(expectedCssLocator, waitLong, 500, true);
		});
	};
	
	CommonFncts.expandCollapseAccordionItemByName = (itemName, expandItem, expectedCssLocator) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		it('click on ' + itemName + ' accordion menu to expand ' + expandItem, async () => {
			...
			const accordionItems = await CommonFncts.findElementsByCSS('details[class*=-form]:not([style*=display]) summary');
				...
				for (let i = 0; i < 30; i++) {
					...
					if (accordionTitleItem === itemName) {
						...
						if (expectedCssLocator) {
							await CommonFncts.waitForObjectLoad(expectedCssLocator, waitLong * 2, 100, true);
						}
	...
	CommonFncts.selectMultipleCheckboxesInAGroup = (checkBoxesGroupLocator, itemsToSelect) => {
		/* -------------------------------------------------------------------------------------------------------------
	...
	CommonFncts.clickLinkFromGroupByName = (linksGroupLocator, linkNameToClick, expectedCssLocator) => {
		...
		it('click ' + linkNameToClick + ' link from the ' + linksGroupLocator + ' links group', async () => {
			...
			if (!groupElements.length > 0) {
				await CommonFncts.waitForObjectLoad(linksGroupLocator, waitLong * 3, 500, true);
			...
			//check if the expected locator is displayed (is truthy or falsy)
			if(expectedCssLocator) {
				await CommonFncts.waitForObjectLoad(expectedCssLocator, waitLong * 5, 500, true);
			}
		});
	};
	...
	CommonFncts.waitForPageLoad = async () => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.waitForPageLoad();
		----------------------------------------------------------------------------------------------------------------- */
	...
	CommonFncts.waitForObjectLoad = async (cssLocator, waitTimeMilliseconds, minWaitMilliseconds, isEnabled) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  await CommonFncts.waitForObjectLoad("#subheader-container", 60000, 500, true);
		----------------------------------------------------------------------------------------------------------------- */
	...
	CommonFncts.validateDisplayedTextEquals = (cssLocator, expectedTextValue) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.validateDisplayedTextEquals('.license-number-notice', 'Note: This Licence number will not be editable once entered.');
		----------------------------------------------------------------------------------------------------------------- */
	...
	CommonFncts.validateDisplayedTextContains = (cssLocator, expectedTextValue) => {
		/* -----------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will validate text of any element
		...
		USAGE:  CommonFncts.validateDisplayedTextContains('a[href*="' + imagePreviewAndLinkLocator + '"]', imagePreviewAndLinkLocator);
		----------------------------------------------------------------------------------------------------------------- */
	...
	CommonFncts.checkErrorsExist = (expectedConditions, expectedErrorText) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.checkErrorsExist(false); - means check there are no errors
				CommonFncts.checkErrorsExist(true, 'some expected error); - means check there are errors
		----------------------------------------------------------------------------------------------------------------- */
		...
		it('check if there are any errors, expected: ' + expectedConditions + (expectedErrorText ? ' -> ' +
			...
			const elements = await CommonFncts.findElementsByCSS('*[class*="error"]');    //also exist: .error && .messages.error
	...
	CommonFncts.clickOnTabByText = (tabNameToClick, waitObjectCssLocator) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.clickOnTabByText('Authorization', 'a[aria-controls="AuthorizationSearchPanelBody"]');
		----------------------------------------------------------------------------------------------------------------- */
		it('find the ' + tabNameToClick + ' tab and click on it', async () => {
			...
			// await CommonFncts.waitForObjectLoad('ul[class*=tabs]', waitLong, 500, true);
			await CommonFncts.waitForObjectLoad('.tabs ul[class*=tabs]', waitLong, 500, true);
			
			//count the # of tabs available within the group above
			let tabElements = await CommonFncts.findElementsByCSS('.tabs ul[class*=tabs]>li>a');
			...
			if (tabsCounter > 0) {
				...
				for (let i = 0; i < 10; i++) {
					...
					if (tabText === tabNameToClick && tabIsActive === true) {
						...
						tabElements = await CommonFncts.findElementsByCSS('.tabs ul[class*=tabs]>li>a');
				...
				await CommonFncts.waitForObjectLoad(waitObjectCssLocator, waitLong * 3, 500, true);
			} else {
	...
	// GENERAL ACTIONS:
	CommonFncts.clickElementByCSS = async (elementCss) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.clickElementByCSS('.preset');
		------------------------------------------------------------------------------------------------------------- */
		await CommonFncts.waitForObjectLoad(elementCss, waitLong, 100, true);
	...
	CommonFncts.clickElementByXPath = async (elementXPath) => {
		/* -------------------------------------------------------------------------------------------------------------
	...
	CommonFncts.mouseHover = async (elementOrCssLocator) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.mouseHover('#constructGroup ul:last-child a.dropdown-toggle');
		----------------------------------------------------------------------------------------------------------------- */
	...
	CommonFncts.mouseScroll = async (elementOrCssLocator, ) => {
		/* -----------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will mouse over an element by the given CSS Locator
	...
	CommonFncts.setTextFieldValueByCSS = async (fieldCss, value) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setTextFieldValueByCSS('*[name="applicant[email]"]', 'testemail@test.com');
		------------------------------------------------------------------------------------------------------------- */
		await CommonFncts.waitForObjectLoad(fieldCss, waitLong, 100, true);
	...
	CommonFncts.setSelectDropDownValueByCSS = async (cssLocator, txtValue) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setSelectDropDownValueByCSS('select[name="parentCondition"]', baseSelectedParentCondition);
		------------------------------------------------------------------------------------------------------------- */
		let element = null;
		...
		//var loopsCounter = 0;
		await CommonFncts.waitForObjectLoad(cssLocator, waitLong * 3, 100, true);
	...
	CommonFncts.setButtonRadioFieldValueByCSS = async (fieldCss, value) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setButtonRadioFieldValueByCSS('*[name="developmentType"]', 'TRANSMISSION PIPELINE');
		------------------------------------------------------------------------------------------------------------- */
		let element;
		const cssPath = fieldCss + "[value='" + value + "']";
		await CommonFncts.waitForObjectLoad(cssPath, waitLong, 100, true);
	...
	CommonFncts.setButtonCheckboxByCSS = async (fieldCss, selectItem) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setButtonCheckboxByCSS('#edit-status-value', true);
		------------------------------------------------------------------------------------------------------------- */
		await CommonFncts.waitForObjectLoad(fieldCss, waitLong, 100, true);
		await driver.wait(until.elementLocated(By.css(fieldCss)), waitLong);
	...
	CommonFncts.getElementValueByCSS = async (elementCss) => {
		/* -------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will look for an object by given CSS Locator
		...
		USAGE:  CommonFncts.getElementValueByCSS('.modal-body');
		------------------------------------------------------------------------------------------------------------- */
		await driver.wait(until.elementLocated(By.css(elementCss)), waitLong);
		return await driver.findElement(By.css(elementCss)).getText();
	};
	
	CommonFncts.findElementsByCSS = async (elementCss) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.findElementsByCSS('*[type="checkbox"]');
		------------------------------------------------------------------------------------------------------------- */
		return await driver.findElements(By.css(elementCss));
	};
	
	CommonFncts.getElementsByCSS = async (elementCss) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.getElementValueByCSS('*[type="checkbox"]');
		------------------------------------------------------------------------------------------------------------- */
		await driver.wait(until.elementLocated(By.css(elementCss)), waitLong);
		return await driver.findElements(By.css(elementCss));
	};
	
	CommonFncts.setElementAttributeByCSS = async (elementCss, attr, value) => {
		const element = await driver.findElement(By.css(elementCss));
		await driver.executeScript("arguments[0].setAttribute('" + attr + "', '" + value + "')", element);
	};
	
	CommonFncts.hideElementByCSS = async (elementCss) => {
		//OLD
		// const element = await driver.findElement(By.css(elementCss));
	...
	CommonFncts.popUpConfirmation = async (message, waitDelay) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.popUpConfirmation("the text to be validated", 3000);
		----------------------------------------------------------------------------------------------------------------- */
		it('warning message is displayed and it\'s text equals to "' + message + '"', async () => {
			// await CommonFncts.waitForPageLoad();
			await CommonFncts.waitForObjectLoad('.messages', waitDelay, 500, true);
			let text = await CommonFncts.getElementValueByCSS('.messages:not(h2):not(button)');
	...
	CommonFncts.setFileUploadByCSS = async (fieldCss, value) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setFileUploadByCSS('#uploadAttachment *[name="fileselect[]"]', attachmentPath);
				CommonFncts.setFileUploadByCSS('#uploadAttachment *[name="fileselect[]"]',
	...
	/* --------- to be modified / used across CommonFncts projects if needed -------- */
	CommonFncts.confirmModalHeader = (headerText) => {
		/* -----------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.confirmModalHeader("Header Text");
		----------------------------------------------------------------------------------------------------------------- */
		it('modal dialog appears with header text: ' + headerText, async () => {
			await CommonFncts.waitForObjectLoad('.ui-dialog', waitShort, 500);
			const text = await CommonFncts.getElementValueByCSS('.ui-dialog span');
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	CommonFncts.setTimeFieldValueByID = async (fieldId, value) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setTimeFieldValueByCSS('id', '1200');
		------------------------------------------------------------------------------------------------------------- */
	...
	CommonFncts.setTypeAheadValueByCSS = async (fieldCss, value) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setTypeAheadValueByCSS("*[name="applicant[email]"]", "testemail@test.com");
		------------------------------------------------------------------------------------------------------------- */
		await CommonFncts.waitForObjectLoad(fieldCss, waitLong, 100, true);
		await driver.wait(until.elementLocated(By.css(fieldCss)), waitLong);
		const element = await driver.findElement(By.css(fieldCss));
	...
	CommonFncts.setSelectFieldValueByCSS = async (fieldCss, txtValue) => {
		/* -------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will find a dropdown list by the given CSS Locator
		...
		USAGE:  CommonFncts.setSelectFieldValueByCSS('.reason-for-sir', 'value to be typed');
		------------------------------------------------------------------------------------------------------------- */
		await CommonFncts.waitForObjectLoad(fieldCss, waitLong * 3, 100, true);
		await driver.wait(until.elementLocated(By.css(fieldCss)), waitLong);
		const element = await driver.findElement(By.css(fieldCss));
		// await driver.wait(until.elementIsVisible(element), waitShort);
		await element.click();
	...
	CommonFncts.setBackGridSelectCheckboxes = async (elementCss, index, values) => {
		/* -------------------------------------------------------------------------------------------------------------
		...
		USAGE:  CommonFncts.setBackGridSelect('.preset', 1, 1);
		------------------------------------------------------------------------------------------------------------- */
		let elements;
		await driver.wait(until.elementLocated(By.css(elementCss)), waitLong);
	...
	CommonFncts.setBackGridSelectText = async (elementCss, index, selectText) => {
		/* -------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will
						then it will
	...
	CommonFncts.setBackGridText = async (elementCss, index, keys) => {
		/* -------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will
						then it will
		...
		USAGE:  CommonFncts.setBackGridText('.preset', 1, 1);
		------------------------------------------------------------------------------------------------------------- */
		let elements;
		await CommonFncts.waitForObjectLoad(elementCss, waitLong, 100);
		await driver.wait(until.elementLocated(By.css(elementCss)), waitLong);
		elements = await driver.findElements(By.css(elementCss));
		await elements[index].click();
		await driver.sleep(500);
	...
	CommonFncts.populateGridElementValue = async (element, value) => {
		/* -------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will
						then it will
		...
		USAGE:  CommonFncts.populateGridElementValue(elements[i], "value to be typed");
				CommonFncts.populateGridElementValue(elements[i], wetlandReplacementGridValuesArray[i]);
		------------------------------------------------------------------------------------------------------------- */
		await element.click();
