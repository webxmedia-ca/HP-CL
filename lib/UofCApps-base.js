...
const UofC = (() => {
	...
	UofC.startApp = async (overrideUrl) => {
		...
		await UofC.waitForObjectLoad('.login-page-boxes', waitLong, 1000, true);
		// await UofC.waitForObjectLoad('a[title*=\'Welcome\']', waitShort, 500, true);
		await UofC.validateDisplayedTextEquals('.accountform.login-box>h1', 'Log in');
	...
	UofC.login = async () => {
		...
		if (harnessObj.getUserRole() === 'craigListUser') {
			...
			await UofC.setTextFieldValueByCSS('#inputEmailHandle', username);
			await UofC.setTextFieldValueByCSS('#inputPassword', password);
			
			//2. click Log In button
			await UofC.clickElementByCSS('#login');
			
			//3. wait for the adds/listings grid to load --- user is logged in
			await UofC.waitForObjectLoad('.account-header>a:nth-child(3)', waitLong, 500, true);
			await UofC.validateDisplayedTextEquals('.account-header>a:nth-child(3)', 'home of listings@hopestreet.ca');
			await UofC.waitForObjectLoad('table[class*=accthp_postings][role=grid]', waitShort, 500, true);
	...
	UofC.mouseScroll = async (elementOrCssLocator, ) => {
		/* -----------------------------------------------------------------------------------------------------------------
		DESCRIPTION:    this function will mouse over an element by the given CSS Locator
