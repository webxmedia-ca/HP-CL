...
const testHarness = (function () {
		...
		if(options.browserStack) {
			if(options.browserStackArgs) {
				...
				if (browserStackArgs.length !== 5) {
					throw "not all (or no) browserStackArgs added, expected 5 instead of " + browserStackArgs.length +
					": \n" + browserStackArgs + '\n';
				}
			}
		}
	...
	const init = async () => {
		...
		if (appName === 'craigListVan') baseUrl = configJson.appsUrls.craigListVan;
	...
	const quit = async (test) => {
		try {
			if (driver) {
				// REPORTING *******************************************************************************************
				/* ----------------------------------->>>>>>>>  DISABLED AS I DON'T WANT THEM FOR NOW - ENABLE IF NEEDED
